...
# Function to load-in required packages:
load_packages <- function(pkgs) {
  ...
  # Load all packages silently into the session
  invisible(lapply(pkgs, library, character.only = TRUE))
}
# Required packages:
req_packages = c('dplyr','tidyr','purrr','NoSleepR','slider','rlang','fuzzyjoin','zoo','data.table','lubridate','scales','broom','seacarb','magrittr','scales',
                 'mltools','Metrics','readxl','lmodel2','stringr','janitor','readr','svDialogs','ggpubr','RColorBrewer','dataCompareR','broom','gridExtra')
load_packages(req_packages)

# Turn on no-sleep setting
nosleep_on(keep_display = FALSE) # keeps system awake but allows screen to turn off
...
############ Jul-Aug 2026 Additional revisions (AWM from NEP_Jul2026review.R file) ###########
# ---- 1. Re-running QA scripts for Barnegat, Casco, and Delaware ----

setwd(local_dir) # set to local directory
save_Odrive_option = 'n'
save_local_option = 'n'
progress_print_option = 'y'
source('qaqc_NEP_main.R') # load in master QAQC script 
data_path = 'O:/PRIV/CPHEA/PESD/NEW/EPA/PCEB/Acidification Monitoring/NEP Acidification Impacts and WQS/Data/4. Finalized Data from NEPs/'
load(paste0(data_path,'data_list.Rdata')) # load pre-revised data

# Re-run QA Script:
source('qaqc_NEP_Barnegat_updated.R')
source('qaqc_NEP_Casco_updated.R')
source('qaqc_NEP_Delaware_updated.R')

# Re-run any relevant blocks from NEP_revisions for Barnegat, Casco, and Delaware:
# Barnegat: 
qa_barnegat = qa_barnegat %>% 
  ...
  select(-Comments, -co2_ppm, -sensor_CO2pro, -sensor_CO2Pro)
# Casco:
#Revision: Flag depths < 0 and alk < 0 as "2"
qa_casco = qa_casco %>% 
  mutate(flags_revision = if_else(
    depth_m < 0 & flags == 1, # if depth is negative & flags == 1
    2,                        # then: make flags_revision = 2
    flags_revision)) %>%      # else: keep same value 
  mutate(flags_revision = if_else(
    alk_mgl < 0 & flags == 1,
...
#Comment: Alk.mgl order of magnitude too high???? I think this is the case or the column name is incorrect.
#Revision: adjusted columns to properly be named alk.umolkg, which aligns with other uses. 
if ('alk_mgl' %in% names(qa_casco)) {
  qa_casco = qa_casco %>% 
    rename(alk_umolkg = alk_mgl)
}
# Delaware:
if('co2_ppm' %in% names(qa_delaware)) {
...
cutoff_date = as.POSIXct('2014-12-31 23:59:59',format='%Y-%m-%d %H:%M:%S', tz='UTC')
pass_barnegat = qa_barnegat %>% filter(datetime_utc > cutoff_date & flags_revision == 1)
...
nep_unfiltered_data$Barnegat = nep_unfiltered_data$Barnegat %>% 
  filter(!is.na(datetime_utc)) 
nep_filtered_data$Barnegat = nep_filtered_data$Barnegat %>% 
  filter(!is.na(datetime_utc))

# Coastal Bend
# Change -999 DO values to NA, rename NEP to official name
nep_unfiltered_data$Coastalbend = nep_unfiltered_data$Coastalbend %>% 
  mutate(do_mgl = NA, # -999 values for do_mgl --> turn into NA to avoid issues
         site_code = 'CB_MA', # rename site_code from 'Coastal Bend' to something else
         NEP = 'Coastal Bend Bays') # rename NEP value from 'Coastal Bend' to 'Coastal Bend Bays'
nep_filtered_data$Coastalbend = nep_filtered_data$Coastalbend %>% 
  mutate(do_mgl = NA, # -999 values for do_mgl --> turn into NA to avoid issues
         site_code = 'CB_MA', # rename site_code from 'Coastal Bend' to something else
         NEP = 'Coastal Bend Bays') # rename NEP value from 'Coastal Bend' to 'Coastal Bend Bays'
...
#  Mobile
# Filter out data that was new and not-yet-QC'd at time of download (2024-onwards)
nep_unfiltered_data$Mobile = nep_unfiltered_data$Mobile %>% 
...
# NYNJH
start_window = as.POSIXct('2016-12-01 00:00:00', tz='UTC')
end_window = as.POSIXct('2017-03-31 00:00:00', tz='UTC')
# df = nep_filtered_data$NYNJH %>% filter(between(datetime_utc, start_window, end_window))

# Assign 'SUSPECT' flags (2) to data out of range after consulting with Brittney Flaten (HRECOS Coordinator)
nep_unfiltered_data$NYNJH = nep_unfiltered_data$NYNJH %>% 
  mutate(flags_revision = case_when(
    format(datetime_utc,'%Y-%m') == '2017-01' & site_code == 'HRPR25H' ~ 2,
    TRUE ~ flags_revision
  )) %>% 
  mutate(flags_revision = case_when(
    between(datetime_utc, as.POSIXct('2018-07-31 13:50:00', tz='UTC'), as.POSIXct('2018-08-28 12:10:00', tz='UTC')) & site_code == 'HRPR84H' ~ 2,
    TRUE ~ flags_revision
  )) %>% 
  mutate(flags_revision = case_when(
    format(datetime_utc,'%Y-%m') == '2017-10' & site_code == 'HRPVSC' & ph_T < 7.2 ~ 2,
    TRUE ~ flags_revision
  ))
# apply to filtered version
nep_filtered_data$NYNJH = nep_unfiltered_data$NYNJH %>% 
  filter(flags_revision == 1 & datetime_utc > cutoff_date)
...
# # ---- 5. Adding .nc data to Long Island Sound ----
# 
# # nc_data_1 = nc_open(paste0(lis_dir,'/WLIS2018_UCONN_BuoyData_CTDObot.nc'))
# # nc_data_2 = nc_open(paste0(lis_dir,'/WLIS2018_UCONN_BuoyData_CTDOsurf.nc'))
# # nc_data_3 = nc_open(paste0(lis_dir,'/WLIS2018_UCONN_BuoyData_pCO2bot.nc'))
# # 
# # ctdo_bot = data.frame(
# #   site_code = 'WLIS_B',
# #   station_name = 'Mooring WLIS bottom',
# #   lat = 40.95583,
# #   lon = -73.58000,
# #   time_DD = ncvar_get(nc_data_1, 'time'),
# #   temp_c = ncvar_get(nc_data_1, 'temp'),
# #   sal_ppt = ncvar_get(nc_data_1,'salt'),
# #   depth_m = ncvar_get(nc_data_1,'press'),
# #   do_mgl = ncvar_get(nc_data_1,'DOconc'),
# #   do_pct = ncvar_get(nc_data_1,'DOsat'),
# #   ph = ncvar_get(nc_data_1,'pH')
# # )
# # ctdo_bot = ctdo_bot %>% 
# #   mutate(datetime_utc = as.POSIXct(ctdo_bot$time_DD * 86400, origin='1970-01-01',tz='UTC'))
# # 
# # 
# # ctdo_surf = data.frame(
# #   site_code = 'WLIS_S',
# #   station_name = 'Mooring WLIS surface',
# #   lat = 40.95583,
# #   lon = -73.58000,
# #   time_DD = ncvar_get(nc_data_2, 'time'),
# #   temp_c = ncvar_get(nc_data_2, 'temp'),
# #   sal_ppt = ncvar_get(nc_data_2,'salt'),
# #   depth_m = ncvar_get(nc_data_2,'press'),
# #   do_mgl = ncvar_get(nc_data_2,'DOconc'),
# #   do_pct = ncvar_get(nc_data_2,'DOsat'),
# #   ph = ncvar_get(nc_data_2,'pH')
# # )
# # ctdo_surf = ctdo_surf %>% 
# #   mutate(datetime_utc = as.POSIXct(ctdo_surf$time_DD * 86400, origin='1970-01-01',tz='UTC'))
# # 
# # pco2_bot = data.frame(
# #   site_code = 'WLIS_pco2',
# #   station_name = 'Mooring WLIS pco2',
# #   lat = 40.95583,
# #   lon = -73.58000,
# #   time_DD = ncvar_get(nc_data_3, 'time'),
# #   depth_m = ncvar_get(nc_data_3,'press'),
# #   pco2_uatm = ncvar_get(nc_data_3,'PCO2WAT')
# # )
# # pco2_bot = pco2_bot %>% 
# #   mutate(datetime_utc = as.POSIXct(pco2_bot$time_DD * 86400, origin='1970-01-01',tz='UTC'),
# #          pco2_ppm = pco2_uatm / (depth_m / 10.1325))
# # 
# # 
# # merged_nc_data = list(ctdo_bot, ctdo_surf, pco2_bot) %>% 
# #   reduce(full_join, by='station_name')
# # 
# # # Packages
# # packages <- c("tidync", "ncdf4", "dplyr", "tidyr", "purrr", "stringr", "lubridate")
# # to_install <- setdiff(packages, rownames(installed.packages()))
# # if (length(to_install)) install.packages(to_install, dependencies = TRUE)
# # invisible(lapply(packages, library, character.only = TRUE))
# # 
# # # Paths and files
# # lis_dir <- "O:/PRIV/CPHEA/PESD/NEW/EPA/PCEB/Acidification Monitoring/NEP Acidification Impacts and WQS/NEP_Monitoring_Project_AWM/Data/Raw Data/Long Island Sound"
# # nc_files <- file.path(
# #   lis_dir,
# #   c("WLIS2018_UCONN_BuoyData_CTDObot.nc",
# #     "WLIS2018_UCONN_BuoyData_CTDOsurf.nc",
# #     "WLIS2018_UCONN_BuoyData_pCO2bot.nc")
# # )
# # 
# # nc_data_1 = nc_open(paste0(lis_dir,'/WLIS2018_UCONN_BuoyData_CTDObot.nc'))
# # 
# # # Helper: convert CF time units ("seconds|minutes|hours|days since YYYY-MM-DD [HH:MM:SS]") to POSIXct
# # cf_time_to_posix <- function(x, units_str, tz = "UTC") {
# #   if (is.null(units_str) || !is.character(units_str) || !grepl("since", units_str, ignore.case = TRUE)) {
# #     return(x) # return as-is if not CF-style
# #   }
# #   # Extract unit and origin
# #   unit <- tolower(str_match(units_str, "(seconds|minutes|hours|days)")[,2])
# #   if (is.na(unit)) {
# #     return(x)
# #   }
# #   origin_date <- str_match(units_str, "(\\d{4}-\\d{2}-\\d{2})")[,2]
# #   origin_time <- str_match(units_str, "(\\d{2}:\\d{2}:\\d{2})")[,2]
# #   if (is.na(origin_date)) return(x)
# #   if (is.na(origin_time)) origin_time <- "00:00:00"
# #   origin <- as.POSIXct(paste(origin_date, origin_time), tz = tz)
# #   mult <- switch(unit,
# #                  "seconds" = 1,
# #                  "minutes" = 60,
# #                  "hours"   = 3600,
# #                  "days"    = 86400,
# #                  NA_real_)
# #   if (is.na(mult)) return(x)
# #   origin + as.numeric(x) * mult
# # }
# 
# 
# 

# ---- 6. Clerical: Re-ordering columns, removing unncessary columns, chronology  ----

# 6a. Remove Pensacola from Dataset 
nep_unfiltered_data$Pensacola = NULL
nep_filtered_data$Pensacola = NULL

# 6b. Ensure each NEP's data is sorted chronologically:
nep_unfiltered_data = lapply(nep_unfiltered_data, function(df) {
  df %>% arrange(datetime_utc)
})
nep_filtered_data = lapply(nep_filtered_data, function(df) {
  df %>% arrange(datetime_utc)
})

# 6c. Create flags_all, remove other flags columns for both nep_unfiltered_data and nep_filtered_data
# first: add flags_revision to Tillamook
nep_unfiltered_data$Tillamook =nep_unfiltered_data$Tillamook %>% 
  mutate(flags_revision = do.call(pmax, c(select(nep_unfiltered_data$Tillamook, starts_with('flags_')),na.rm=TRUE)))
nep_filtered_data$Tillamook =nep_filtered_data$Tillamook %>% 
  mutate(flags_revision = do.call(pmax, c(select(nep_filtered_data$Tillamook, starts_with('flags_')),na.rm=TRUE)))

# RENAME ALL flags_revision to flags_all here *****

# 6d. Dropping unncessary columns
# Define the columns to drop
cols_to_drop <- c("sal_class_median", "site_id_std", 'timezone','Project','Humidity','Humidity_sensor_temperature','Cell_gas_pressure', 'datetime_est',
                  'Battery_voltage','Count','DATE','TIME','datetime_string','interval','datetime_us/central','SC','timestamp_local','timestamp_est',
                  'flags_revision_test','do_sat_limit','DateTimeStamp','datetime_parsed','datetime_cdt','record','EXPOCODE','STATION_NO','NISKIN_NO',
                  'MONTH_UTC','DAY_UTC','YEAR_UTC','datetime_pst','QF_PRES(IRGA)','PAR (microEinsteins)','QF_PAR','SIGMATHETA_KG_M3','file') # remove unnecesasry 'flags' or 'flags_revision' here
# Clean the data downloaded from O:drive
nep_unfiltered_data <- lapply(nep_unfiltered_data, function(df) {
  remaining_cols <- setdiff(names(df), cols_to_drop)
  
  if (inherits(df, "data.table")) {
    return(df[, remaining_cols, with = FALSE])
  } else {
    return(df[, remaining_cols, drop = FALSE])
  }
})
nep_filtered_data <- lapply(nep_filtered_data, function(df) {
  remaining_cols <- setdiff(names(df), cols_to_drop)
...
reorder_nep_data = function(data) {
  # fail safely if the input is not a data frame
  if (!is.data.frame(data)) {
    warning('Input is not a data frame, skipping.')
    return(data)
  }
  # Apply column priority logic
  data %>% 
    select(
      any_of(c('datetime_utc','NEP','station_name','site_code','lat','lon','temp_c','sal_ppt','depth_m','ph_T','do_mgl','do_pct')),
      starts_with('co2_'),
      starts_with('alk_'),
      any_of(c('region','season','sensor')),
      starts_with('sensor_'),
      any_of('flags_all'),
      everything() # grabs all remaining columns in their current order
    )
}
...
nosleep_off() # turn off no-sleep setting
